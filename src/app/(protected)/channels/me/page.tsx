...
export default function FriendsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const response = await friendAPI.getRelationships();
  ...
  useEffect(() => {
    (async () => {
      try {
        const response = await friendAPI.getIncomingRelationships();
        if (Array.isArray(response.data)) {
          setIncomingRelationships(response.data);
        } else {
          console.error("API response is not an array:", response);
          setIncomingRelationships([]);
        }
  ...
  useEffect(() => {
    (async () => {
      try {
        const response = await friendAPI.getOutgoingRelationships();
        if (Array.isArray(response.data)) {
          setOutgoingRelationships(response.data);
        } else {
          console.error("API response is not an array:", response);
          setOutgoingRelationships([]);
        }
  ...
  return (
    <div className="flex flex-col h-full bg-[#313338]">
      ...
      >
        ...
        {/* Search Bar - Only show in lists, not in add friend tab */}
        ...
        <ScrollArea className="flex-1">
          ...
          <TabsContent value="all" className="m-0 p-4">
            <div className="space-y-1">
              {relationships.map((relationship: RelationshipsOutput) => {
                const user: User = relationship.otherUser;
                return (
                  <FriendItem
                    key={user.id}
                    name={user.username}
          ...
          <TabsContent value="pending" className="m-0 p-4">
            <div className="space-y-4">
              {incomingRelationships.length > 0 && (
                <div>
                  <h3 className="text-xs font-semibold text-[#B5BAC1] mb-2 px-2">
                    PENDING — {incomingRelationships.length}
                  </h3>
                  <div className="space-y-[2px]">
                    {incomingRelationships.map(
                      (incomingRelationship: RelationshipsOutput) => {
                        const user: User = incomingRelationship.otherUser;
                        return (
                          <PendingFriendItem
                            key={user.id}
                            name={user.username}
              ...
              {outgoingRelationships.length > 0 && (
                <div>
                  <h3 className="text-xs font-semibold text-[#B5BAC1] mb-2 px-2">
                    SENT REQUESTS — {outgoingRelationships.length}
                  </h3>
                  <div className="space-y-[2px]">
                    {outgoingRelationships.map(
                      (outgoingRelationship: RelationshipsOutput) => {
                        const user: User = outgoingRelationship.otherUser;
                        return (
                          <PendingFriendItem
                            key={user.id}
                            name={user.username}
