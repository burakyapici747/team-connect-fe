...
export default function DirectMessagePage({params}: {
    ...
    // useMessages hook'u channelId'ye göre mesajları çekiyor ve sendMessage fonksiyonunu sağlıyor.
    const {messages, isLoading, error, sendMessage} = useMessages(params.channelId);
    ...
    // WebSocket bağlantısı: Yeni mesaj geldiğinde query cache'i güncelliyoruz.
    ...
    // useMessages hook'undan gelen sendMessage fonksiyonu kullanılarak mesaj gönderiliyor.
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        try {
            setIsSending(true);
            // Burada artık channelId'yi tekrar göndermemize gerek yok,
            // useMessages hook channelId'yi zaten sabitliyor.
            await sendMessage({
    ...
    {isGroupChat ? "Uçan Kuş" : "Kullanıcı Adı"}
                            ...
                            messages.map((message, index) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {isPartOfGroup && (
                                            <div
                                                className="absolute left-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-100">
            <span
                className="text-xs"
        ...
        {new Date(message.timestamp).toLocaleDateString("tr-TR", {
            ...
            minute: "2-digit",
                                        ...
                                        <div
                                            className="absolute right-2 top-0 hidden group-hover:flex items-center gap-0.5 py-1 px-0.5 rounded bg-[#313338] shadow-md">
                                            <Button
                                                variant="ghost"
                                                size="icon"
