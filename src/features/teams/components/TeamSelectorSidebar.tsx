...
export function TeamSelectorSidebar() {
  ...
  const { currentUserTeams, isLoading, error } = useTeams();
  ...
  const getImageUrl = (icon: string) => {
    ...
    return `${process.env.NEXT_PUBLIC_API_URL}/uploads/${icon}`;
  };
        ...
        <div className="flex-1 w-full overflow-y-auto space-y-3 px-3 scrollbar-hide">
          {isLoading ? (
              <div className="flex justify-center">
                <div className="w-[48px] h-[48px] rounded-[24px] bg-[#313338] animate-pulse" />
              </div>
          ) : (
                    ...
                    <Tooltip delayDuration={50}>
                      <TooltipTrigger asChild>
                        <Button
                            ...
                            onClick={() => router.push(`/channels/${team.id}`)}
                        >
                          {team.icon ? (
